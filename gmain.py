...
class MainW(Gtk.Window):
	def __init__(self):
		...
		self.bks=self.__getImpostazioni(self.fconf)
		#print(self.bks)
		self.lst_lbl=[]
		self.lst_chiavi=[]
		...
		#oggetti
		self.lstMain=self.__attachLista()
		grid.add(self.lstMain)		
		grid.attach(self.__attachButton(),0,1,1,1)		
	...
	def on_modifica_clicked(self,bt):
		...
		w=DlgConf(self.bks[self.lst_chiavi[row.get_index()]])
		w.connect("destroy", Gtk.main_quit)
		w.set_modal(True)
		w.show_all()
		Gtk.main()
